// nfl-week1-embed-anywhere.js — drop this in and it will render Week 1 (2025) on the page.
// Usage: <script src="nfl-week1-embed-anywhere.js" defer></script>
(function(){ "use strict";
...
.mc-footnote{margin:.6rem 0 0 0;color:#aab3ff;font:.85rem/1.4 system-ui}`;
  function injectCSS(){
    ...
    try{
      ...
      // CSP fallback: try external CSS if provided
      if(!document.querySelector('link[href$="mc-sports.css"]')){
        var l=document.createElement('link'); l.rel='stylesheet'; l.href='mc-sports.css'; document.head.appendChild(l);
      }
    }
  }
...
${cards}
  ...
  function findMount(){
    ...
    if(el) return el;
    // 2) headers with text "NFL"
    var hs = Array.from(document.querySelectorAll('h1,h2,h3,h4'));
    for (var i=0;i<hs.length;i++){ var t=(hs[i].textContent||'').trim().toLowerCase(); if(t==='nfl') return hs[i]; }
    // 3) nav links to #nfl
    var a = document.querySelector('a[href="#nfl"]'); if(a) return a;
    // 4) fallback: body end
  ...
  ready(function(){
    ...
    if(mount===document.body){
      ...
      mount.insertAdjacentElement('afterend', wrapper.firstElementChild);
    }
  });
})();